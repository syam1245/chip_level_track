...
const JobSheetPrintTemplate = forwardRef(({ item }, ref) => {
  ...
  const InfoBlock = ({ title, data }) => (
    <Box sx={{ mb: 2 }}>
      <Typography sx={{ fontSize: '12px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', mb: 1, letterSpacing: '0.5px' }}>
        {title}
      </Typography>
      {data.map((row, i) => (
        <Box key={i} sx={{ display: 'flex', mb: 0.5 }}>
          <Typography sx={{ width: '100px', fontSize: '13px', color: '#475569', fontWeight: 500 }}>{row.label}:</Typography>
          <Typography sx={{ fontSize: '14px', color: '#0f172a', fontWeight: 600 }}>{row.value || '—'}</Typography>
        </Box>
      ))}
    </Box>
  );
  ...
  return (
    ...
    >
          ...
          <TableBody>
            ...
            <TableRow>
              <TableCell sx={{ py: 3, pl: 0, verticalAlign: 'top', borderBottom: '1px solid #e2e8f0' }}>
                <Typography sx={{ fontSize: '14px', fontWeight: 600, color: '#1e293b', mb: 1 }}>
                  Diagnostic & Service Notes
                </Typography>
                <Typography sx={{ fontSize: '13px', color: '#475569', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>
                  {item.repairNotes || "Standard diagnosis and repair service. No special technical notes provided at this time."}
                </Typography>
              </TableCell>
              <TableCell align="right" sx={{ py: 3, verticalAlign: 'top', borderBottom: '1px solid #e2e8f0', fontSize: '14px', fontWeight: 600 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 2 }}>
          <Box sx={{ width: '250px' }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
              <Typography sx={{ fontSize: '14px', color: '#475569' }}>Subtotal:</Typography>
              <Typography sx={{ fontSize: '14px', color: '#0f172a' }}>{item.cost ? `₹${item.cost}` : '—'}</Typography>
            </Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
              <Typography sx={{ fontSize: '14px', color: '#475569' }}>Tax:</Typography>
              <Typography sx={{ fontSize: '14px', color: '#0f172a' }}>—</Typography>
            </Box>
            <Divider sx={{ mb: 1, borderColor: '#cbd5e1' }} />
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Typography sx={{ fontSize: '16px', fontWeight: 800, color: '#0f172a' }}>TOTAL:</Typography>
              <Typography sx={{ fontSize: '18px', fontWeight: 800, color: accentColor }}>
                {item.cost ? `₹${item.cost}` : 'TBD'}
              </Typography>
            </Box>
          </Box>
      ...
      <Box sx={{ mt: 'auto', pt: 4 }}>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
          {[
            ...
            <Typography key={index} sx={{ fontSize: '11px', color: '#64748b', width: '48%' }}>
              • {text}
            </Typography>
          ))}
        </Box>
      ...
      <Box sx={{ mt: 8, display: 'flex', justifyContent: 'space-between' }}>
        {['Customer Signature', 'Authorized Signatory'].map((label, i) => (
          <Box key={i} sx={{ width: '220px', textAlign: 'center' }}>
            ...
            <Typography sx={{ fontSize: '12px', fontWeight: 600, color: '#475569', textTransform: 'uppercase' }}>
              {label}
            </Typography>
          </Box>
        ))}
      ...
      {/* --- FOOTER --- */}
      <Typography sx={{ position: 'absolute', bottom: '15mm', left: '50%', transform: 'translateX(-50%)', fontSize: '11px', color: '#94a3b8', fontWeight: 500 }}>
